...
// =====================================================================
// 1. DATA BOUNDARIES & VERBATIM SNIPPETS (Verbatim from Framework)
// =====================================================================
const DATA_BOUNDARIES = {
    "confidence": { low: [5, 11], medium: [12, 18], high: [19, 30] },
    "self_awareness": { low: [6, 13], medium: [14, 22], high: [23, 30] },
...
// =====================================================================
// 2. STATE ENGINE VARIABLE TRACKERS
// =====================================================================
let currentQuestionIndex = 0;
let userResponses = {};
...
function startAssessment() {
    ...
    if (!nameVal || !schoolVal) {
        alert("Please complete Name and School profile metrics before launching.");
        return;
    }
...
function renderCurrentQuestion() {
    ...
    // Restore answers safely or map to standard neutral default configuration index
    const previouslySavedWeight = userResponses[prompt.id];
    const radioInputs = document.getElementsByName("response");
    
    let defaultRadioIndexValue = 2; // Neutral index point default fallback
    if (previouslySavedWeight !== undefined) {
        let rawScoreWeightValue = previouslySavedWeight;
        if (prompt.reverse) {
            rawScoreWeightValue = 6 - previouslySavedWeight;
...
// =====================================================================
// 4. REPORT MATRIX GENERATOR LOGIC
// =====================================================================
function compileAndDisplayReportDashboard() {
    ...
    // Array of tab panel mapping keys
    const dimRefs = [
        { key: "confidence", id: "card-m-conf", targetTab: "tab-confidence", title: "CONFIDENCE & EXPRESSION" },
        { key: "self_awareness", id: "card-m-aware", targetTab: "tab-awareness", title: "SELF-AWARENESS" },
        { key: "decision_making", id: "card-m-dec", targetTab: "tab-decision", title: "DECISION-MAKING STYLE" },
        { key: "emotional_balance", id: "card-m-bal", targetTab: "tab-balance", title: "EMOTIONAL BALANCE" },
        { key: "drive", id: "card-m-drive", targetTab: "tab-drive", title: "DRIVE & FUTURE ORIENTATION" }
    ];
    ...
    dimRefs.forEach(d => {
        ...
        // Update main landing matrix column containers cleanly
        ...
        // Programmatically generate and build the internal tab blocks dynamically
        let goodListItems = chunk.good.map(i => `<li>${i}</li>`).join("");
        let improveListItems = chunk.improve.map(i => `<li>${i}</li>`).join("");
        let actionItemCards = chunk.next.map((step, idx) => `
            <div class="action-item-card">
        ...
        const panelElement = document.getElementById(d.targetTab);
        if (panelElement) {
    ...
    // Generate Dynamic Insight Text Content
    document.getElementById("overall-analysis-text").innerHTML = `
        Your profile demonstrates a powerful developmental intersection. Your strongest foundational behavioral asset maps directly to your execution score in 
        <b>${dimTitles[highestKey]}</b>, which should be leveraged as an ongoing catalyst tool. Concurrently, your primary strategic improvement priority 
        is situated within developing consistency inside <b>${dimTitles[lowestKey]}</b>. Closing this tracking gap will optimize your operational pacing.
    ...
    // Transition panels smoothly out of quiz to final report screen state
    document.getElementById("quiz-screen").classList.remove("active");
    document.getElementById("dashboard-screen").classList.add("active");
    
    // Jump straight to window focus anchor top view point
